...
AllLoop.prototype = {
    ...
    adjustStyle: function(type) {
        ...
        if (type === 3) {
            ...
            var max_vLineheight = max_maxVlineLength * 80; //顶部最高危急值高度
            h -= max_vLineheight;
            for (var i = 0; i < sections.length; i++) {
                var maxVlineLength = sections[i].getAttribute('maxVlineLength');
...
DataAdapter.prototype = {
    ...
    getColumnParam: function(data) {
        ...
        function handleData(data) {
            ...
            var count = 0;
			var maxNode = 0;
			var showCount = this.o.showCount; // 是否展示次数，按照count展示，count为对象的“下标”
			// 根据执行记录id排序
            ...
            for (var k in group) {
                ...
                lineDatas.forEach(function(lineData, index) {
					...
					var subtitle = groupField && !showCount ? k : '第' +(t_index)+ '次执行';
